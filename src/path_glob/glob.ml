...
module NFA =
  struct
    ...
    module IS = Set.Make(struct type t = int let compare (x:t) y = compare x y end);;
    module ISM = Map.Make(struct type t = IS.t let compare = IS.compare end);;
...
type globber = fast_pattern atom Formula.t;;
(* ***)
...
let add_dir dir x =
  ...
  | Some(dir) ->
      match x with
      | Constant(s) ->
          Constant(Filename.concat dir s)
      | Pattern(p) ->
          Pattern(Concat(Word(Filename.concat dir ""), p))
;;
(* ***)
(*** add_ast_dir *)
