...
let read_symbols ?(dot_merlins=[]) ic toc =
    ...
    (
      try%lwt
        while%lwt true do
          ...
          if%lwt Lwt.return (String.is_prefix ~affix:"B " line ) then (
            let path = line |> String.with_index_range ~first:2 |> String.trim in
            let path =
  ...
  Lwt.return {
    global_table;
    event_by_pc;
    all_dirs = String_set.to_seq !all_dirs |> List.of_seq;
    module_info_tbl;
  }
...
let path_to_modname (path : string) : string =
  String.Ascii.capitalize Filename.(path |> basename |> remove_extension)
